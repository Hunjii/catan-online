...
// Check all 54 vertices
vertices.forEach(v => {
  const connectedEdges = edges.filter(e => e.vertexIds.includes(v.id));
  console.log(`Vertex ${v.id} (hexes: ${v.hexIndices}): connectedEdges=${connectedEdges.length}, IDs=${connectedEdges.map(e => e.id)}`);
});
