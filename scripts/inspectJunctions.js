...
// Find the 3 hexes meeting at the center:
// In Row 1: hex 4 ({ q: 0, r: -1 })
// In Row 2: hex 8 ({ q: -1, r: 0 }), hex 9 ({ q: 0, r: 0 })
// Let's find vertices shared by 3 hexes
vertices.forEach(v => {
  if (v.hexIndices.length === 3) {
    const adjEdges = edges.filter(e => e.vertexIds.includes(v.id));
    console.log(`Vertex ${v.id} (pos: ${v.position.x.toFixed(2)}, ${v.position.z.toFixed(2)}): hexes=[${v.hexIndices}], adjacentEdgeIds=[${v.adjacentEdgeIds}], edges found=${adjEdges.map(e => `${e.id}(${e.vertexIds[0]}->${e.vertexIds[1]})`)}`);
  }
});
