import type { Config } from "tailwindcss";

const config: Config = {
  content: [
    "./src/pages/**/*.{js,ts,jsx,tsx,mdx}",
    "./src/components/**/*.{js,ts,jsx,tsx,mdx}",
    "./src/app/**/*.{js,ts,jsx,tsx,mdx}",
  ],
  theme: {
    extend: {
      colors: {
        catan: {
          wood: "#2E7D32",
          brick: "#C62828",
          sheep: "#8BC34A",
          wheat: "#FBC02D",
          ore: "#546E7A",
          desert: "#D7CCC8",
          ocean: "#0288D1",
          gold: "#FFB300",
          cardboard: "#EFEBE9",
          "dark-wood": "#3d2314",
          "light-wood": "#5c3a21",
          "parchment": "#f4e4bc",
          "gold-trim": "#bfa054"
        },
      },
      backgroundImage: {
        'wood-pattern': "url('/assets/wood-bg.jpg')",
        'dark-wood-pattern': "url('/assets/dark_wood_bg.jpg')",
        'parchment-pattern': "url('/assets/parchment-bg.jpg')",
      },
      boxShadow: {
        'inset-wood': 'inset 0 2px 10px rgba(0,0,0,0.8), 0 5px 15px rgba(0,0,0,0.5)',
        'btn-wood': '0 4px 6px rgba(0,0,0,0.5), inset 0 2px 4px rgba(255,255,255,0.2)',
        'btn-wood-active': '0 1px 2px rgba(0,0,0,0.5), inset 0 4px 6px rgba(0,0,0,0.6)',
      },
      fontFamily: {
        cinzel: ["var(--font-cinzel)", "Cinzel", "Georgia", "serif"],
        "cinzel-decorative": ["var(--font-cinzel-decorative)", "Cinzel Decorative", "Cinzel", "Georgia", "serif"],
        cormorant: ["var(--font-cormorant)", "Cormorant Garamond", "Garamond", "Georgia", "serif"],
        grenze: ["var(--font-grenze-gotisch)", "Grenze Gotisch", "Georgia", "serif"],
        "alegreya-sc": ["var(--font-alegreya-sc)", "Alegreya SC", "Georgia", "serif"],
        vietnam: ["var(--font-be-vietnam)", "var(--font-inter)", "system-ui", "sans-serif"],
        catan: ["var(--font-cinzel)", "var(--font-playfair)", "Georgia", "serif"],
        sans: ["var(--font-be-vietnam)", "var(--font-inter)", "system-ui", "sans-serif"],
      },
    },
  },
  plugins: [],
};
export default config;
